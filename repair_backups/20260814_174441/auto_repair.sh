...
log ""
log "[10/10] Automatic Windows-assisted JPEG repair..."

JPEG_PARENT="$PROJECT/.buildozer/android/platform/build-arm64-v8a/build/bootstrap_builds/sdl2/jni/SDL2_image/external"
JPEG_DIR="$JPEG_PARENT/jpeg"
...
if command -v powershell.exe >/dev/null 2>&1; then
    ...
    WIN_TMP="$(
        powershell.exe -NoProfile -Command \
        '[System.IO.Path]::GetTempPath()' 2>/dev/null |
        tr -d '\r\n'
    )"

    WIN_TMP="${WIN_TMP:-C:\\Windows\\Temp}"

    log "Windows temporary directory: $WIN_TMP"

    WIN_ARCHIVE_WIN="$WIN_TMP\\abg-jpeg-v9e-SDL.zip"
    WIN_EXTRACT_WIN="$WIN_TMP\\abg-jpeg-v9e-SDL"

    WIN_ARCHIVE_WSL="$(
        wslpath "$WIN_ARCHIVE_WIN" 2>/dev/null || true
    )"

    WIN_EXTRACT_WSL="$(
        wslpath "$WIN_EXTRACT_WIN" 2>/dev/null || true
    )"

      log "Downloading SDL2 JPEG source through Windows..."

      POWERSHELL_SCRIPT='
        $ErrorActionPreference = "Stop"
        $url = "https://github.com/libsdl-org/jpeg/archive/refs/heads/v9e-SDL.zip"
        $archive = "'"$WIN_ARCHIVE_WIN"'"
        $extract = "'"$WIN_EXTRACT_WIN"'"
        ...
        Write-Output "WINDOWS_JPEG_DOWNLOAD_START"
          ...
          if (-not (Test-Path -LiteralPath $archive)) {
              throw "JPEG ZIP was not downloaded"
          }

          Write-Output "WINDOWS_JPEG_ARCHIVE=PASS"
        ...
        if (-not (Test-Path -LiteralPath $extract)) {
            throw "JPEG archive extraction failed"
        }

        Write-Output "WINDOWS_JPEG_EXTRACT=PASS"
      '

      powershell.exe -NoProfile -Command "$POWERSHELL_SCRIPT" 2>&1 | tee -a "$LOG"

      if grep -q "WINDOWS_JPEG_ARCHIVE=PASS" "$LOG" &&
         grep -q "WINDOWS_JPEG_EXTRACT=PASS" "$LOG"; then
          echo "WINDOWS_JPEG_DOWNLOAD=PASS" >> "$LOG"
      else
          log "WINDOWS_JPEG_DOWNLOAD=FAIL"
      fi

      if grep -q "WINDOWS_JPEG_DOWNLOAD=PASS" "$LOG"; then

          log "Windows successfully downloaded JPEG."

          EXTRACTED_DIR=""

          # 1. Known Windows temporary extraction path
          KNOWN_WIN_JPEG="/mnt/c/Users/Emd/AppData/Local/Temp/abg-jpeg-test"

          if [ -d "$KNOWN_WIN_JPEG" ]; then
              log "Checking known Windows extraction path..."
              EXTRACTED_DIR="$(
                  find "$KNOWN_WIN_JPEG" -maxdepth 2 -type d \
                  -name "jpeg-*" -print -quit 2>/dev/null
              )"
          fi

          # 2. Dynamically detected Windows temporary path
          if [ -z "$EXTRACTED_DIR" ] && [ -d "$WIN_EXTRACT_WSL" ]; then
              log "Checking detected Windows extraction path..."
              EXTRACTED_DIR="$(
                  find "$WIN_EXTRACT_WSL" -maxdepth 3 -type d \
                  -name "jpeg-*" -print -quit 2>/dev/null
              )"
          fi

          # 3. Final safe fallback: search only inside Windows Temp
          if [ -z "$EXTRACTED_DIR" ]; then
              log "Searching Windows Temp for extracted JPEG..."
              EXTRACTED_DIR="$(
                  find /mnt/c/Users/Emd/AppData/Local/Temp \
                  -maxdepth 4 -type d -name "jpeg-*" \
                  -print -quit 2>/dev/null
              )"
          fi

          if [ -z "$EXTRACTED_DIR" ]; then
              log "JPEG source was not found in Windows temporary directory."
              log "WIN_EXTRACT_WSL=$WIN_EXTRACT_WSL"
          else
              log "JPEG source located successfully:"
              log "$EXTRACTED_DIR"
          fi

        if [ -n "$EXTRACTED_DIR" ] && [ -d "$EXTRACTED_DIR" ]; then

            log "Found extracted JPEG source:"
            log "$EXTRACTED_DIR"

            mkdir -p "$JPEG_PARENT"

            if [ -e "$JPEG_DIR" ]; then
                SAFE_JPEG_BACKUP="$BACKUP/jpeg_existing_$(date +%H%M%S)"
                log "Existing JPEG directory detected."
                log "Moving it safely to:"
                log "$SAFE_JPEG_BACKUP"
                mv "$JPEG_DIR" "$SAFE_JPEG_BACKUP"
            fi

            cp -a "$EXTRACTED_DIR" "$JPEG_DIR"

            if [ -d "$JPEG_DIR" ]; then
                log "LOCAL JPEG SOURCE INSTALL: PASS"
            else
                log "LOCAL JPEG SOURCE INSTALL: FAIL"
            fi

        else
            log "Could not locate extracted JPEG source."
        fi

    else
        log "WINDOWS JPEG DOWNLOAD: FAIL"
    fi

else
    log "PowerShell not available."
fi

log ""
log "[11/11] Verifying local JPEG source..."

if [ -d "$JPEG_DIR" ]; then

    log "JPEG directory exists:"
    log "$JPEG_DIR"

    if [ -f "$JPEG_DIR/CMakeLists.txt" ] || \
       [ -f "$JPEG_DIR/Makefile" ] || \
       [ -f "$JPEG_DIR/configure" ]; then

        log "JPEG source verification: PASS"
        JPEG_READY=1

    else
        log "JPEG source verification: UNKNOWN"
        log "Directory exists but expected build files were not detected."
        JPEG_READY=0
    fi

else
    log "JPEG source verification: FAIL"
    JPEG_READY=0
fi

log ""
log "============================================================"
log " WINDOWS-ASSISTED REPAIR RESULT"
log "============================================================"

if [ "${JPEG_READY:-0}" -eq 1 ]; then
    log "JPEG dependency has been supplied locally."
    log ""
    log "The next build can now retry SDL2_image without"
    log "requiring WSL to clone github.com/libsdl-org/jpeg.git."
else
    log "Automatic JPEG repair was not completed."
fi

log ""
log "IMPORTANT:"
log "No .buildozer directory was deleted."
log "No ~/.buildozer directory was deleted."
log "No NDK was changed."
log "No Kivy version was changed."
log "No python-for-android version was changed."
